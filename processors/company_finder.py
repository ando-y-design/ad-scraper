...
def _normalize_name(name: str) -> str:
    ...
    # All rights reserved（英文著作権表示）を除去
    # 例: "SBS holdings Inc. All rights reserved." → "SBS holdings Inc."
    name = re.sub(r'\s*[,.]?\s*[Aa]ll\s+[Rr]ights?\s+[Rr]eserved\.?\s*$', '', name).strip()
    # 「会社概要」はこちら 等のナビリンク文言を除去
    # 例: "株式会社 ZENB JAPAN 「会社概要」はこちら" → "株式会社 ZENB JAPAN"
    name = re.sub(r'\s*「[^」]{1,20}」は(?:こちら|こちらから)?\s*$', '', name).strip()
    # ホームページ末尾直結を除去
    # 例: "自然派くらぶ生活協同組合ホームページ" → "自然派くらぶ生活協同組合"
    name = re.sub(r'ホームページ\s*$', '', name).strip()
    # 代表者名以降（役職・氏名）を除去
    # 例: "株式会社キューブ代表者名代表取締役会長兼社長松村智明（コード番号" → "株式会社キューブ"
    name = re.sub(r'代表者名.+$', '', name).strip()
    # 求人・バイト系の文字列が含まれる場合、先頭の法人名だけを抽出
    # 例: "のぞみ税理士法人の大阪市エリアの事務STAFFのバイト・アルバイト求人情報" → "のぞみ税理士法人"
    if re.search(r'(?:バイト|アルバイト|パートタイム|派遣社員|派遣スタッフ)', name) and len(name) > 15:
        m = re.search(r'(?:法人|株式会社|有限会社|合同会社|医療法人|税理士法人|司法書士法人|弁護士法人|社会福祉法人|NPO法人|協同組合)', name)
        if m:
            candidate = name[:m.end()]
            if 4 <= len(candidate) < len(name) - 3:
                name = candidate
    # の+採用/求人/個人情報/会社概要等の末尾説明を除去（スペースなし直結）
    # 例: "株式会社アクセスの採用・求人情報" → "株式会社アクセス"
    # 例: "株式会社サンフジ企画の個人情報のお取り扱いについて" → "株式会社サンフジ企画"
    name = re.sub(
        r'の(?:採用(?:情報|サイト|ページ|・求人情報)?|求人(?:情報|サイト|ページ)?|'
        r'個人情報[^$]*について|プライバシーポリシー[^$]*|'
        r'お取り扱い[^$]*|会社(?:概要|情報|案内|紹介))\s*$',
        '', name
    ).strip()
    # 閉じカッコなし（末尾まで続く）の括弧注記を除去
    # 例: "ICONIC Co.,Ltd.（グローバル転職のアイコニック" → "ICONIC Co.,Ltd."
    name = re.sub(r'\s*[（(][^）)]{4,}$', '', name).strip()
    # 末尾のハイフン・アンダースコア・スラッシュ等を除去
    name = re.sub(r'[\-‐–—－_/\\|｜・]+$', '', name).strip()
    # 末尾の孤立した閉じ括弧（対応する開き括弧がない場合のみ）を除去。
    # 対応括弧 例:「（東京本社）」「(株式会社○○)」は後続のペア処理で扱うため保持する。
    ...
    # 末尾の【会社名】から会社名を抽出（前置き説明+【会社名】のパターン）
    # 例: "単調な工場作業はイヤだ！【株式会社八王子】" → "株式会社八王子"
    m_bracket = re.search(
        r'【((?:株式会社|有限会社|合同会社|合資会社|合名会社|一般社団法人|公益社団法人|'
        r'一般財団法人|公益財団法人|医療法人|学校法人|弁護士法人|税理士法人|司法書士法人|'
        r'社会福祉法人|NPO法人|特定非営利活動法人|協同組合)[^】]+)】\s*$',
        name
    )
    if m_bracket and not name.startswith(('株式会社', '有限会社', '合同会社', '一般社団法人',
                                           '医療法人', '学校法人', 'NPO法人', '協同組合')):
        name = m_bracket.group(1).strip()
    # 前置き説明を除去: 「内装工事の株式会社○○」「説明 株式会社○○」→「株式会社○○」
